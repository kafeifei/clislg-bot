...
export interface LeaderboardData {
  world: { seasonId: string; turn: number; status: string; phase: string; elapsedSeconds: number };
  summary: { joinedPlayers: number; stalledPlayers: number; interventionCandidates: number };
  myPlayer?: {
    topGuideTask: string;
    stuckReason: string;
  ...
  battles: { turn: number; attackerId: string; result: string; attackerLosses: number; defenderLosses: number }[];
}
