...
export async function replenishTroops(client: ApiClient, armySlot?: number): Promise<unknown> {
  ...
  return client.post('/army/replenish', armySlot !== undefined ? { armySlot } : undefined);
}

export async function assignGenerals(client: ApiClient, assignments: { generalId: string; position: number }[], armySlot?: number): Promise<unknown> {
  log.info(`分配将领${armySlot !== undefined ? ` (军队${armySlot})` : ''}`);
  return client.post('/army/assign', { assignments, ...(armySlot !== undefined && { armySlot }) });
}
...
export async function startMarch(client: ApiClient, params: {
  ...
  log.info(`行军出发 → (${params.targetQ},${params.targetR},${params.targetS}) 意图:${params.intent || 'occupy'}`);
  return client.post('/march/start', params);
}
